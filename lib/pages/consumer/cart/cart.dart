        ...
        ? Scaffold(
            body: ListView.builder(
              ...
              itemBuilder: (context, index) {
                String title =
                    result.elementAt(index).first.values![0].toString();
                String description =
                    result.elementAt(index).first.values![1].toString();
                String price =
                    result.elementAt(index).first.values![2].toString();
                return subserviceCard(
                  title: title,
                  description: description,
                  price: price,
                  onRemove: () async {
                    bool isDeleted = await databaseService.deleteCart(
                        email,
                        result.elementAt(index).first.values![3].toString(),
                        result.elementAt(index).first.values![4].toString());
                    if (isDeleted == true) {
                      showTopSnackBar(
                        Overlay.of(context),
                    ...
                    ElevatedButton(
                      onPressed: () async {
                        ...
                        for (int i = 0; i < result.length; i++) {
                          String serviceId =
                              result.elementAt(i).first.values![3].toString();
                          String subservice =
                              result.elementAt(i).first.values![4].toString();

                          await databaseService.addToOrders(
                              email, getCurrentDate(), serviceId, subservice);

                          // delete the cart after ordering the items .
